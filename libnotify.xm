...
uint32_t replaced_notify_post(const char *name) {
  if (hookNotifyPost){
    if (nil != filter){
      NSString *pSTRING=[[NSString alloc] initWithUTF8String:name];
...
int  replaced_sqlite3_prepare_v2(
  ...
  if (hookSqlite){
    if (nil != filter){
      NSString *sqString=[[NSString alloc] initWithUTF8String:zSql ];
...
//   }
  ...
  // Constructor
  // %ctor {
  //   UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"fuck"
  //   message:@"get called"
  //   delegate:nil
